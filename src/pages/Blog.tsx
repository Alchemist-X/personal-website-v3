  ...
  return (
    <div className="min-h-screen p-8 pb-24">
      <div className="max-w-4xl mx-auto">
        <h1 className="text-5xl font-bold text-terminal-text mb-4">Blog</h1>
        ...
        <p className="text-terminal-text mb-8 underline">
        ...
        <div className="space-y-6">
          {posts.map((post) => (
            <div
              key={post.id}
              className="bg-terminal-bg border border-terminal-border rounded-lg p-6 hover:border-terminal-hover transition-all hover:shadow-lg"
            >
              <div className="flex justify-between items-start mb-3">
                <span className="text-[var(--muted-text)] text-sm">{post.date}</span>
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {post.tags.map((tag, tagIndex) => (
                  <span
                    key={tagIndex}
                    className="px-3 py-1 rounded-full text-sm font-medium bg-terminal-hover text-terminal-text hover:opacity-80 transition-colors cursor-pointer"
                  >
                    {tag}
                  </span>
                ))}
              ...
              <Link
                to={`/blog/${post.id}`}
                className="inline-flex items-center text-terminal-text hover:text-[var(--link-color)] transition-colors"
              >
                read more &gt;
              </Link>
            </div>
